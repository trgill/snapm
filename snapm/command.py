...
def _mount_cmd(cmd_args):
    ...
    Individual member mount points can be overridden with --mount-point.
    ...
    # Parse mount point overrides from --mount-point arguments
    ...
    manager.mounts.mount(
        snapset, mount_root=mount_root, mount_points=mount_points_override or None
    )
    return 0
...
def _add_snapset_subparser(type_subparser):
    ...
    snapset_mount_parser.add_argument(
        ...
        dest="mount_points",
        default=None,
        help="Override mount point for a snapshot set member (can be specified multiple times)",
    )
